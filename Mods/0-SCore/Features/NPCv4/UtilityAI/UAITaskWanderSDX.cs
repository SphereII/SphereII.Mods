...
namespace UAI
{
    public class UAITaskWanderSDXV4 : UAITaskWander
    {
        private string _interest;

        // Task instances are singletons: one UAITaskWanderSDXV4 object is created per <task> XML
        ...
        {
            CombatUtils.SetCrouching(_context);
            ...
            if (!string.IsNullOrEmpty(_interest))
            {
                ...
                if (rand < 0.3)
                {
                    var paths = ContainerUtils.ScanForTileEntities(_context, _interest);
                    if (paths.Count > 0)
                    {
            ...
            PathingUtils.FindPath(_context, position, false);
        }
        ...
        public override void Update(Context _context)
        {
            ...
            if (_redirectCooldowns.TryGetValue(entityId, out int cooldown) && cooldown > 0)
            {
                ...
                PathingUtils.CheckJump(_context);
                if (hadPath && _context.Self.getNavigator().noPathAndNotPlanningOne())
                {
                    ...
                    {
                        PathingUtils.FindPath(_context, currentTarget, false);
                    }
                }
            }
            else
            {
                // Two independent ways this gets stuck: PathingUtils.IsBlocked() (collision/raycast -
                ...
                blocked = PathingUtils.IsBlocked(_context) || _context.Self.getNavigator().noPathAndNotPlanningOne();
            }
            ...
            if (blocked)
            {
                ...
                PathingUtils.FindPath(_context, newPosition, false);
